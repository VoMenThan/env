...
class Errors {
	...
	public function admin_init() {
		...
		if ( ! empty( $_GET['refresh_errors'] ) ) {
			...
			rank_math()->add_error( esc_html__( 'Errors list refreshed.', 'rank-math' ), 'success' );
		...
		if ( ! empty( $_GET['maf_uri'] ) && ! empty( $_GET['maf_category'] ) ) {
			...
			if ( $response ) {
				...
				rank_math()->add_error( esc_html__( 'Marked as fixed: ', 'rank-math' ) . '<code>' . esc_html( $_GET['maf_uri'] ) . '</code>', 'success' );
			}
		}
