...
class Shortcodes {
	...
	public function contact_info( $args ) {
		...
		wp_enqueue_style( 'rank-math-contact-info', rank_math()->assets() . 'css/rank-math-contact-info.css', null, rank_math()->get_version() );
	...
	private function display_social() {
		...
		<div class="rank-math-social-networks">
			...
			foreach ( $networks as $id => $label ) :
				if ( $url = Helper::get_settings( 'titles.social_url_' . $id ) ) : // @codingStandardsIgnoreLine
					?>
