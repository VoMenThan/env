...
class Setup_Wizard {
	...
	public function admin_page() {
		...
		// Enqueue javascript.
		wp_enqueue_script( 'rank-math-wizard', rank_math()->plugin_url() . 'assets/admin/js/wizard.js', array( 'media-editor', 'select2-rm', 'rank-math-common' ), rank_math()->get_version(), true );
	...
	protected function searchconsole_form() {
		...
		$this->cmb->add_field( array(
			...
			'default'    => GlobalHelper::get_settings( 'general.console_profile' ),
			'after'      => '<button class="button button-primary" ' . ( $data['authorized'] ? '' : 'disabled="disabled"' ) . '>' . esc_html__( 'Refresh Sites', 'rank-math' ) . '</button>',
	...
	public function role_form() {
		...
		foreach ( GlobalHelper::get_roles() as $role => $label ) {
			$this->cmb->add_field( array(
				'id'      => esc_attr( $role ),
				...
				'classes' => 'cmb-big-labels' . ( count( $defaults[ $role ] ) === $cap_count ? ' multicheck-checked' : '' ),
			) );
		}
	}
	...
	public function misc_form() {
		...
		foreach ( GlobalHelper::get_accessible_post_types() as $post_type ) {
			...
			$article_desc = 'person' === GlobalHelper::get_settings( 'titles.knowledgegraph_type' ) ? '<div class="notice notice-warning inline" style="margin-left:0;"><p>' . sprintf( __( 'Google does not allow Person as the Publisher for articles. Organization will be used instead. You can read more about this <a href="%s" target="_blank">here</a>.', 'rank-math' ), 'https://developers.google.com/search/docs/data-types/article/?utm_campaign=Rank+Math' ) . '</p></div>' : '';
			$this->cmb->add_field( array(
