...
$wp_version_ok = version_compare( $wp_version, rank_math()->wp_version, '>' );
			...
			<th>
				...
				if ( $php_version_ok ) {
					...
					if ( $php_version_recommend ) {
						?>
						<?php echo ' | ' . esc_html__( 'Recommended: PHP 7.1 or later', 'rank-math' ); ?>
						<p class="description">
						<?php
							echo ( ! Helper::is_whitelabel() ) ?
								esc_html__( 'Rank Math is compatible with your PHP version but we recommend updating to PHP 7.1 for increased speed and security.', 'rank-math' ) . ' <a href="' . KB::get( 'rm-requirements' ) . '" target="_blank">' . esc_html__( 'More information', 'rank-math' ) . '</a>' :
								esc_html__( 'This plugin is compatible with your PHP version but we recommend updating to PHP 7.1 for increased speed and security.', 'rank-math' );
						?>
						</p>
						<?php
					}
					...
					/* translators: php version */
					printf( esc_html__( 'Your PHP Version: %s | Recommended version: 7.0 | Minimal required: 5.4', 'rank-math' ), $php_version );
				}
				?>
			</th>
