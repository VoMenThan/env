...
class Head {
	...
	public function __construct() {
		...
		if ( Helper::is_module_active( 'amp' ) ) {
			$this->action( 'amp_post_template_head', 'head', 1 );
	...
	public function publisher() {
		if ( $publisher = Helper::get_settings( 'titles.social_url_gplus' ) ) { // @codingStandardsIgnoreLine
	...
	private function credits( $closing = false ) {
		...
		if ( false === $closing ) {
			if ( ! Helper::is_whitelabel() ) {
				echo "\n<!-- " . esc_html__( 'Search Engine Optimization by Rank Math - https://mythemeshop.com/plugins/wordpress-seo/', 'rank-math' ) . " -->\n";
			}
			return;
		}
